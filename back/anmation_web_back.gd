...
func parse_motion(root: String, bone_names: Array, bone_index_map: Dictionary, 
	...
	for line in text:
		...
		for bone_index in range(len(bone_names)):
			...
			# 对于根骨骼，还需要处理位置
				# 创建根骨骼的变换
			#var rotation=bvh_to_godot_quaternion([raw_rotation.x,raw_rotation.y,raw_rotation.z],"XYZ")
			...
			if bone_index == 0:
				...
				# 对于非根骨骼，只需添加旋转关键帧
				#var rerot=Quaternion(Vector3.UP,-0.5)
				#rotation=rerot*rotation
...
#				以下是解析丑陋的实现，线性代数不太好
				if bone_id>0:
					...
					var localTrans=root_transform*trans
					match remapped_bone_names[bone_index]:

						"LeftLowerLeg","RightLowerLeg":
							#rotation=localTrans.basis.get_rotation_quaternion()
							rotation=localTrans.basis.get_rotation_quaternion()
							var r=rotation.get_euler()
							r.x=-r.x
							r.y=-r.y
							rotation=Quaternion.from_euler(r)
							
							#var t=Transform3D().rotated(Vector3.LEFT,deg_to_rad(-90))
							#rotation=localTrans.rotated_local(Vector3.LEFT,deg_to_rad(-90)).basis.get_rotation_quaternion()
							pass
						"LeftUpperArm":
							rotation=bvh_to_godot_quaternion([raw_rotation.x,raw_rotation.y,raw_rotation.z],"YZX")
							root_transform.basis=Basis(rotation)
							localTrans=root_transform*trans
							rotation=localTrans.basis.get_rotation_quaternion()
							#rotation=localTrans.basis.rotated(Vector3.UP,deg_to_rad(90)).get_rotation_quaternion()
							pass
						"RightUpperArm":
							rotation=bvh_to_godot_quaternion([raw_rotation.x,raw_rotation.y,raw_rotation.z],"YZX")
							root_transform.basis=Basis(rotation)
						...
						"RightHand":
							...
							rotation=bvh_to_godot_quaternion([raw_rotation.x,raw_rotation.y,raw_rotation.z],"YXZ")
							root_transform.basis=Basis(rotation)
							localTrans=root_transform*trans
							var r=localTrans.basis.get_euler()
							r.y=-PI-r.y
						...
						_:
							if step==0:
								get_test_angle(raw_position,trans,remapped_bone_names[bone_index])
							rotation=localTrans.basis.get_rotation_quaternion()
